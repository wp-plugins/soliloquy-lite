	...
	public function contextual_help() {
		...
		if ( 'edit-soliloquy' == $current_screen->id && Tgmsp_Lite::is_soliloquy_screen() ) {
			$current_screen->add_help_tab( array(
				...
				'content'	=> sprintf( '<p>%s</p><p>%s</p><p><strong>%s</strong></p>', Tgmsp_Lite_Strings::get_instance()->strings['main_help'], Tgmsp_Lite_Strings::get_instance()->strings['main_help_two'], sprintf( Tgmsp_Lite_Strings::get_instance()->strings['upgrade_nag'], sprintf( '<a href="http://soliloquywp.com/pricing/?utm_source=orgrepo&utm_medium=link&utm_campaign=Soliloquy%2BLite" title="%1$s" target="_blank">%1$s</a>', Tgmsp_Lite_Strings::get_instance()->strings['upgrade_nag_link'] ), '' ) )
			) );
		}
